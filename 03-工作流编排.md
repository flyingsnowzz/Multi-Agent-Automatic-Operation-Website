...
```mermaid
flowchart TB
    CHIEF["主编/运营负责人<br/>确定主题方向"] --> TOPIC
    START["⏰ 定时触发<br/>每日8:00"] --> TOPIC
    TOPIC["🔍 选题Agent<br/>扩展选题/关键词"]
    APPROVE{"主编确认主题?"}
...
上图中的  节点在 crawler 内容链路中由 **QualityAgent** 实现。
QualityAgent 会在两处运行：

1. **原文质量评分**：crawler 原文  后，QualityAgent 判断原文写作质量。
   -  → 进入 ResearchAgent + WriterAgent 改写
   -  → 直接进入发布候选

2. **生成稿复评**：WriterAgent 生成文章后，QualityAgent 按  权重复评。
   -  → 通过，进入 EditorAgent
   -  → 触发重试循环（最多 5 次）
