...
2. **门禁评估** - 评估素材是否可以进入文章重要性 Agent
3. **决策引擎** - 决定内容处理方式（discard/pass_to_importance_agent）
...
- `decision_rules` - 决策规则（仅 `discard` / `pass_to_importance_agent`）

## 输出

- `processing_result` - 处理结果（discard/pass_to_importance_agent）
- `next_payload` - 传递给文章重要性 Agent 的素材 payload（如果决策为 pass_to_importance_agent）
...
5. **路由** - 仅保留两种结果：
   - `pass_to_importance_agent`：标记为通过门禁，传递给文章重要性 Agent
...
- **上游**：爬虫数据库（外部系统）
- **下游**：
  - 如果决策为 `pass_to_importance_agent` → 文章重要性 Agent
  - 如果决策为 `discard` → 结束
...
- crawler 的目标架构不再以 `TopicAgent` 作为正式下游，而是以“文章重要性 Agent”作为下一层。

## 配置依赖

- **数据库连接**：爬虫数据库、已发布内容数据库
- **LLM 配置**：可选，仅用于补充评估解释，不作为 crawler 核心职责
- **决策规则**：丢弃条件与 `pass_to_importance_agent` 条件
...
for item in result.get("items", []):
    if item.get("decision") == "pass_to_importance_agent":
