...
当前生产系统通过 **系统 Cron** 或 **手动执行** 运行：

```bash
# 手动运行（5 篇，可改 target 变量）
python3 scripts/run_final.py

# Cron 定时运行（每天早上 8 点）
0 8 * * * cd /path/to/project && python3 scripts/run_final.py >> logs/cron.log 2>&1
```

> 注：以下 APScheduler 方案为 yaoyao 设计的通用平台调度系统（位于 `yaojiayk/scheduler/`），
> 当前生产环境未启用。作为扩展参考保留。
