...
第一版不建立关键词数据库，也不为关键词划分重要性等级。所有关键词由运营人员在独立配置文件中手动维护，系统将配置中的关键词同等作为热点搜索范围。

建议配置文件为 `config/active_research_keywords.yaml`：
...
配置分组只用于管理和日志追踪，不代表权重或优先级。第一版修改关键词时直接编辑该文件；后续如果需要后台管理，再增加自动维护能力。

## 4. 选题评分

每个热点候选计算独立的 `topic_score`，该分数不与现有爬虫文章的 `ai_score` 混用。

`topic_score` 评价的是搜索结果中的新闻事件是否值得写，不评价关键词本身。关键词之间没有高、中、低等级，也不配置不同权重。
...
“完整质量链路”表示一篇文章只有通过全部必要节点才能进入 `pending`，不表示每个候选都必须成功完成。任何节点返回明确失败条件时，本篇任务立即中断，记录失败节点和原因，然后继续处理下一篇候选。
...
关键词不写入数据库，只读取外部 YAML 配置。主动研究候选不写入爬虫文章表；为避免无人值守运行时重复处理，可以使用以下独立状态表。
...
第一阶段如果只进行少量人工测试，也可以先用本地 JSON 状态文件代替两张候选表。正式无人值守运行前应切换到数据库状态记录，保证进程重启后仍能去重和续跑。
...
- 单篇候选在任何节点失败时立即中断本篇，不继续消耗后续 Agent 和图片额度。
- 一篇失败不应终止整批任务；系统记录原因后继续处理下一篇。
...
# 等同于上面命令，输出到 output/active_research_candidates.json。
...
1. 从 Git 历史恢复 TopicAgent 中有价值的关键词、趋势和排序逻辑。第一版已落地为 `ActiveResearchAgent` 的关键词读取、RSS 抓取、去重和规则评分。
2. 重写为 `ActiveResearchAgent`，避免与当前 ScoringAgent 混淆。当前入口为 `scripts/run_active_research.py --discover-only`。
3. 新建独立 LangGraph 和 `scripts/run_active_research.py`。脚本已先支持发现候选，后续再接完整 LangGraph。
