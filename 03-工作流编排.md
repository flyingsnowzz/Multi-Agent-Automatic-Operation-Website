...
2. **生成稿复评**：WriterAgent 生成文章后，QualityAgent 按 `if_ai_generated=true` 权重复评。
   ...
   - `quality_score < 85` → 触发重试循环（最多重写1次；1次后仍不达标直接放弃）
...
crawler_processor = Agent(role="爬虫内容处理器", goal="读取待处理内容并交接给审查链路", ...)
...
async def crawler_process_node(state):
    """爬虫处理节点 — 读取爬虫数据库，完成入口处理并交接"""
    result = await run_crawler_workflow(
        limit=10,
        target_keywords=state.get("keywords", []),
        dry_run=False,
    )
    return {"crawler_result": result}
