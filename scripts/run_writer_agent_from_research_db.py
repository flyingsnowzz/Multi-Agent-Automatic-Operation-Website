...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--loop",
        action="store_true",
        help="Keep running batches until no eligible candidates remain",
    )
    parser.add_argument(
        "--regenerate",
        action="store_true",
...
async def main() -> None:
    ...
    total = {"read": 0, "generated": 0, "failed": 0, "batches": 0}
    while True:
        result = await generate_articles_from_research_db(
            db_config,
            limit=args.limit,
            concurrency=args.concurrency,
            only_missing_outputs=True if args.loop else not args.regenerate,
        )
        total["batches"] += 1
        total["read"] += int(result.get("read") or 0)
        total["generated"] += int(result.get("generated") or 0)
        total["failed"] += int(result.get("failed") or 0)
        print(json.dumps({"batch": total["batches"], **result}, ensure_ascii=False, indent=2))
        if not args.loop or int(result.get("read") or 0) == 0:
            break
        if int(result.get("generated") or 0) == 0 and int(result.get("failed") or 0) > 0:
            break
    if args.loop:
