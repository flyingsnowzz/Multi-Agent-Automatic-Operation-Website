...
flowchart TB
    ...
    AGENT_LAYER --> TOPIC --> RESEARCH --> WRITER --> QUALITY
    QUALITY -->|"质量<70 重写"| RESEARCH
    QUALITY -->|"质量>=85 通过"| EDITOR --> SEO
    SERVICE_LAYER --> CMS_API & IMG
    STORAGE_LAYER --> MYSQL & REDIS
...
| **数据库服务器** | 8核32G 500G SSD | 16核64G 1T SSD | MySQL |
...
QUALITY_AGENT_API_KEY=sk-xxx  # QualityAgent 评分用
QUALITY_AGENT_MODEL=deepseek-chat
...
# 导入初始表结构
mysql -u pipeline -p content_pipgeline < docs/sql/init_schema.sql
```
...
# 测试单个Agent
python -m agents.scoring_agent.main --test
...
1. **读写分离**：MySQL配置主从复制
2. **MySQL分区/归档**：按发布时间、抓取时间或状态分区，冷数据归档
...
# 上传到华为云 OBS（可选，需提前配置 obsutil）
obsutil cp $BACKUP_DIR/mysql_$DATE.sql.gz obs://my-bucket/backup/
