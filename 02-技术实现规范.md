...
flowchart TB
    ...
    CHIEF --> TOPIC
    TOPIC --> RESEARCH --> WRITER --> EDITOR --> SEO --> IMAGE --> CMS_AGENT

    SPIDER --> CRAWLER_DB
    CRAWLER_DB --> CRAWLER --> REVIEW
    REVIEW -->|"发表<br/>ready_to_publish"| CMS_AGENT
    REVIEW -->|"AI改写再创作<br/>ready_to_rewrite"| REWRITE_BRIEF --> WRITER
...
flowchart TD
    ...
    BRIEF --> WRITER["WriterAgent<br/>改写再创作"]
    WRITER --> EDITOR["EditorAgent<br/>审校质量"]
...
|------|-------|----------|----------------|------------|
| 主动调研生产 | TopicAgent | 关键词、运营方向 | 热点、搜索意图、竞争度、站内缺口 | 选题列表 → ResearchAgent |
