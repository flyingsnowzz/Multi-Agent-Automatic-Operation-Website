...
爬虫处理流与主流程并行运行，由定时调度器独立触发：

```mermaid
flowchart TB
    CRAWL_START["⏰ 定时触发<br/>每日多次"] --> CRAWL["🕷️ 爬虫处理Agent<br/>读取待处理内容"]
    CRAWL --> EVAL{"内容评估"}
    EVAL -->|"质量低/重复"| DISCARD["❌ 丢弃<br/>记录原因"]
    EVAL -->|"质量高"| DIRECT["📋 CMS Agent<br/>直接发布"]
    EVAL -->|"质量中等"| REWRITE["✍️ WriterAgent<br/>改写"]
    REWRITE --> EDITOR["🔧 编辑Agent"]
...
def crawler_process_node(state):
    ...
    for item in pending["data"]:
        ...
        if eval_result["quality_score"] >= 90:
            # 高质量 → 直接发布
            cms_agent.execute(item)
        elif eval_result["quality_score"] >= 40:
            # 中等质量 → 改写
            writer_agent.execute_rewrite(item, eval_result)
        # 低质量 → 丢弃（已由evaluator处理）
    return {}
