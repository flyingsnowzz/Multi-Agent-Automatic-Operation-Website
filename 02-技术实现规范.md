...
flowchart TD
    ...
    CRAWLER --> HANDOFF["标准化交接<br/>基础校验通过"]
    HANDOFF --> DEDUP["去重检测<br/>对比已发布内容"]
...
| 主动调研生产 | ImageAgent | SEO稿、主关键词、内容类型 | 封面图/插图生成、Alt文本、多风格提示词 | ImageResult → CozeSite/CMSAgent |
...
| 爬虫筛选分发 | CrawlerProcessorAgent | MySQL爬虫结果库 `status=pending` | 读取、结构标准化、基础字段校验、统一交接 | 标准化素材 → Review |
...
| 项目 | 规格 |
|------|------|
| **核心工具** | `image_generator` / `alt_text_generator` / `coze_image_generator` / `image_prompt_generator` / `prompt_to_image_generator` |
| **输入** | SEO 优化后文章 + 主关键词 + 内容类型 |
| **输出** | ImageResult: featured_image_url/alt/prompt + inline_images + license |
| **封面图** | 1200×630（OG 推荐 1.91:1），DALL-E gpt-image-1 / Coze Site |
| **文中插图** | 1-4 张，800×600 或 1024×1024 |
| **Alt文本** | 25-125 字符，中/英文支持，自然包含主关键词 |
| **提示词生成** | DeepSeek 自动分析文章内容，从 8 种视觉风格中选出最合适的 5-6 种 |
| **生图Pipeline** | 审核文章(DB) → DeepSeek分析 → 提示词入库 → Coze生图 → 图片下载 → 写回DB |
...
### 3.11 CrawlerProcessorAgent 爬虫入口处理

| 项目 | 规格 |
|------|------|
| **核心工具** | `crawler_db_reader` |
| **输入** | MySQL 爬虫结果库中 `status=pending` 的记录 |
| **核心分析** | 标题/正文/来源链接等基础字段是否齐全，输入结构是否可安全交接 |
| **评分维度** | 无；本层不承担去重和评分 |
| **决策规则** | 基础结构合法 → `handoff_to_review`；基础字段异常 → `error` |
| **输出** | 标准化素材、交接信息、基础校验结果 |
| **下游** | Review |
| **LLM模型** | 无 |
...
| **视觉生成** | image_generator, alt_text_generator, coze_image_generator, image_prompt_generator, prompt_to_image_generator | ImageAgent |
