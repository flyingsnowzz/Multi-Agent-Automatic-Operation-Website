...
```mermaid
flowchart TB
    CHIEF["主编/运营负责人<br/>确定主题方向"] --> SCORE
    START["⏰ 定时触发<br/>每日8:00"] --> SCORE
    SCORE["📊 文章评分Agent<br/>文章价值评分"]
    APPROVE{"主编确认主题?"}
    RESEARCH["📚 调研Agent<br/>深度调研+生成大纲"]
    WRITER["✍️ 写作Agent<br/>生成文章初稿"]
    EDITOR["🔧 编辑Agent<br/>审校润色+质量评分"]
    QUALITY{"质量达标?"}
    IMAGE["🎨 配图Agent<br/>生成/选择配图"]
    SEO["🔍 SEO Agent<br/>SEO优化"]
    CMS["📋 CMS Agent<br/>发布/入草稿"]
    SOCIAL["📢 社交分发Agent<br/>多平台分发"]
    DATA["📈 DataAgent<br/>发布后表现分析"]
    RECHECK{"需要再审查优化?"}
    ...
    QUALITY -->|"是"| SEO --> IMAGE --> CMS --> SOCIAL --> DATA --> RECHECK
    RECHECK -->|"内容问题"| EDITOR
    RECHECK -->|"SEO问题"| SEO
    RECHECK -->|"新主题机会"| CHIEF
    RECHECK -->|"无需处理"| DONE["✅ 持续监控"]

    style APPROVE fill:#fff9c4,stroke:#f57f17
    style QUALITY fill:#fff9c4,stroke:#f57f17
    style RECHECK fill:#fff9c4,stroke:#f57f17
```

### 3.1.2 质量评分节点说明

上图中的 `QUALITY{"质量达标?"}` 节点在 crawler 内容链路中由 **QualityAgent** 实现。
QualityAgent 会在两处运行：

1. **原文质量评分**：crawler 原文 `article_score > 75` 后，QualityAgent 判断原文写作质量。
   - `quality_score < 70` → 进入 ResearchAgent + WriterAgent 改写
   - `quality_score >= 85` → 直接进入发布候选

2. **生成稿复评**：WriterAgent 生成文章后，QualityAgent 按 `if_ai_generated=true` 权重复评。
   - `quality_score >= 85` → 通过，进入 EditorAgent
   - `quality_score < 85` → 触发重试循环（最多重写1次）

详见：[[07-QualityAgent说明]]

### 3.1.1 爬虫内容筛选分发流

爬虫内容筛选分发流与主动调研生产流并行运行，由定时调度器独立触发：

```mermaid
flowchart TB
    SPIDER["外部爬虫/采集任务"] --> CRAWLER_DB[("爬虫结果库<br/>status=pending")]
    CRAWL_START["⏰ 定时触发<br/>每日多次"] --> CRAWL["🕷️ CrawlerProcessor<br/>读取待处理内容"]
    CRAWLER_DB --> CRAWL
    CRAWL --> DEDUP["去重检测<br/>对比已发布内容"]
    DEDUP --> SCORE["审查评分<br/>质量/相关性/SEO潜力/版权风险"]
    SCORE --> DECISION{"分发决策"}
    DECISION -->|"发表<br/>ready_to_publish"| DIRECT["📋 CMS Agent<br/>发布/入草稿"]
    DECISION -->|"AI改写再创作<br/>ready_to_rewrite"| BRIEF["生成改写简报"]
    BRIEF --> REWRITE["✍️ WriterAgent<br/>改写再创作"]
    REWRITE --> EDITOR["🔧 EditorAgent<br/>审校质量"]
    EDITOR --> SEO2["🔍 SEO Agent"]
    SEO2 --> CMS2["📋 CMS Agent<br/>发布"]
    DECISION -->|"废弃<br/>discarded"| DISCARD["❌ 放弃/归档<br/>记录评分与原因"]
...
flowchart TB
    ...
    DECISION -->|内容缺口| SCORE["📊 文章评分Agent<br/>评分新文章"]
...
crawler_processor = Agent(role="爬虫内容处理器", goal="评估爬虫内容并决策分发", ...)
...
image_task = Task(
    description="为文章生成封面图和插图（支持 DALL-E / Coze Site），并生成 SEO 友好 Alt 文本",
    agent=image_agent,
    expected_output="ImageResult（featured_image_url/alt/prompt，inline_images，license）",
    context=[editing_task]  # 和 SEO ∥ Image 并行
)
...
def crawler_process_node(state):
    """爬虫处理节点 — 读取爬虫数据库，评估内容，决策分发"""
    pending = crawler_processor.read_pending(limit=10)
    for item in pending["data"]:
        # 去重检测
        dup = dedup_checker.check(item["title"], item["content"])
        if dup["is_duplicate"]:
            continue
        # 质量评估
        eval_result = content_evaluator.evaluate(
            item["title"], item["content"],
            target_keywords=state.get("keywords", [])
        )
        if eval_result["decision"] == "publish":
            # 发表 → ready_to_publish
            cms_agent.execute(item)
        elif eval_result["decision"] == "rewrite":
            # AI改写再创作 → ready_to_rewrite
            writer_agent.execute_rewrite(item, eval_result)
        # 废弃 → discarded（记录评分与原因）
    return {}
...
_相关文档：[[00-方案概述]] | [[01-Agent架构图]] | [[04-定时任务方案]] | [[workflows/]]_
