...
class ResearchAgent:
    ...
    def _rewrite_constraints(self, normalized: Dict[str, Any], risk_points: List[str]) -> List[str]:
        ...
        constraints = [
            ...
            "人物/校友/创业故事只能写源素材明确给出的经历、时间、产品、融资、引用和成果；不要补写办公室、团队规模、第一款产品、融资经过、采访回忆、课程调整或具体年限。",
            "如果源素材只是概括性介绍，不要扩写成完整创业传记；可以写背景意义和已知事实之间的关系，但必须避免虚构剧情细节。",
        ]
        if risk_points:
    ...
    def _style_instruction(self) -> Dict[str, Any]:
        return {
            ...
            "style_name": "编辑改写稿",
            "goal": "写成一篇自然但正式的编辑改写稿。不要像是在逐条执行提示词，也不要写成网络口语、自媒体鸡汤或过度戏剧化的人物故事。",
            "rules": [
                "大纲只是材料整理顺序，不必逐段照搬；可以合并、调序或弱化某些小节，让文章读起来像自然写成的稿子。",
                "默认采用正式媒体报道/校友报道口吻：清楚、平实、有信息密度；可以自然，但不能俚语化、段子化或公众号爆款化。",
                ...
                "人物和创业类稿件尤其要克制：没有原文证据时，不要写租办公室、凑钱、第一款产品、后台数据、融资、回校演讲、课程调整、团队扩张数字或某人回忆。",
                "如果材料只有校友身份、公司名称和结果，文章应该写成事实型校友报道，不要写成带场景的创业传记。",
                "人物稿可以有叙述感，但措辞要稳：不要用“开挂”“踩坑”“塞进”“火苗”“谁知道呢”“这条路不算短”这类网络化或随笔式表达。",
                ...
                "允许信息密度不均衡：某个关键细节可以多写，枯燥背景可以一笔带过，不要让每段都像 100-150 字的均匀模块；常规稿件全文控制在 5-8 个自然段。",
            ],
            "avoid_patterns": [
                ...
                "把缺少细节的校友材料扩写成完整创业故事，添加原文没有的时间线、场景、商业数据或人物发言。",
                "为了显得自然而使用网络口语、调侃句、随笔式结尾或过度戏剧化表达。",
            ],
            "soft_avoid_phrases": [
                "一路开挂",
                "坑他一个没落下",
