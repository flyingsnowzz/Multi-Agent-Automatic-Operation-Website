...
async def save_audit_node(state: ArticleGraphState) -> ArticleGraphState:
    ...
    if status == "ai_score_blocked":
        # First-gate rejects are intentionally not persisted to pipeline_audit.
        # In production, run_langgraph_batch.py can still mark the source row as
        # used, so low-score articles do not keep reappearing in future scans.
        out["cms_status"] = status
        out["audit_persisted"] = False
        _append(out, "warnings", "audit_skipped_ai_score_below_threshold")
        return out
